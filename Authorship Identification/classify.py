...
def count_not_vowel(s):
    return len(re.findall('[^aeiuo]', s, re.IGNORECASE))
...
class FeatureExtractor:
    ...
    def features(self, text):
        d = defaultdict(int)
        k_text = [v for v in kTOKENIZER.tokenize(text) if v not in punctuation]
        for ii in k_text:
            d[morphy_stem(ii)] += 1
        text = [v for v in nltk.word_tokenize(text) if v not in punctuation]
        d['length'] = len(text)
        d['vowels'] = sum(map(count_vowels, text))
        d['consonants'] = sum(map(count_not_vowel, text))
        bigram_list = list()
        # for item in nltk.bigrams(text):
        #     bigram_list.append(" ".join(item))
        # print bigram_list
        # exit()
